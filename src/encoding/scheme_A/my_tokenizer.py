...
class PianoRollTokenizer:
    ...
    def compress_tokens(
        ...
        Args:
            ...
            track_marker: 开始标记（如果为None，则不添加标记）

        Returns:
            compressed_sequence: 压缩后的一维序列

        编码格式：
            非空: [track_marker] [abs_pos0] [token0] [abs_pos1] [token1] ...
                  - 每个音符使用绝对位置编码
            空:   [track_marker,0]
        """
    ...
    def decompress_tokens(
        ...
        """
        解压缩token序列（绝对位置编码 → token矩阵）

        Args:
            compressed_sequence: 压缩的token序列
            track_marker_id: 开始标记（如果为None，则自动检测）
        ...
        while i < len(compressed_sequence):
            ...
            if current_token == track_marker_id:
                ...
                while i < len(compressed_sequence):
                    ...
                    # 检查是否是下一个beat的开始（start_marker或empty_marker）
