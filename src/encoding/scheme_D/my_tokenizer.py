"""
Piano Roll Tokenizer - 钢琴卷帘编解码器 (absolute_bundled 方案 - Scheme D)

统一管理piano roll与token之间的编解码逻辑，包括：
- Patch-based tokenization (三进制编码)
- 绝对位置捆绑编码 (bundled = abs_pitch × 81 + val)
- 双通道piano roll重建

与 with_pair (Scheme C) 的区别：
- compress_tokens: 使用绝对位置而非相对位置
- decompress_tokens: 直接解码绝对位置，无需状态累积
"""
...
class PianoRollTokenizer:
    ...
    def encode(
        ...
        """
        完整编码流程：piano roll → compressed tokens (绝对位置捆绑编码)
    ...
    def decode(
        ...
        """
        完整解码流程：compressed tokens → piano roll (绝对位置捆绑编码)
    ...
    def compress_tokens(
        ...
        """
        使用绝对位置捆绑编码压缩token序列（绝对位置 × pattern_num + patch值）

        与 with_pair (Scheme C) 的区别：使用绝对位置而非相对位置，
        每个bundled token独立编码，无需跟踪前一个位置。
    ...
    def decompress_tokens(
        ...
        """
        解压缩绝对位置捆绑编码的token序列 → token矩阵

        与 with_pair (Scheme C) 的区别：直接从bundled token解码绝对位置，
        无需累积相对位置。
