...
class ModelConfig:
    ...
    # 相对位置编码 (no_pair_related)
    # 0-80: patch token values (三进制编码)
    # 81-168: position markers (81 + relative_distance, max distance=87)
    # 169: empty_marker (空beat)
    # 170: end_marker (非空beat结束)
    # 171+: 特殊token
    vocab_size: int = 184
    patch_h = 1  # 音高方向的patch大小
    patch_w = 4  # 时间方向的patch大小

    pattern_num = 81   # 位置标记起始偏移 (position_marker = 81 + relative_pos)
