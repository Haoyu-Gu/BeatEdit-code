"""
GECToR Dataset and Collator

Loads npz piano roll files, tokenizes to no_pair_related encoding,
...
# Import PianoRollTokenizer from the Scheme B encoding module
# (<repo>/src/encoding/scheme_B/my_tokenizer.py) without polluting sys.path
_SRC_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
MUSIC_BERT_DIR = os.path.join(_SRC_DIR, 'encoding', 'scheme_B')
_spec = importlib.util.spec_from_file_location(
    "encoding_scheme_B_tokenizer",
...
class GECToRDataset(Dataset):
    """
    GECToR training dataset.

    Each __getitem__ dynamically:
    1. Loads npz and encodes to token sequence
    ...
    def __init__(
        ...
        # Create tokenizer (matching music_bert config)
    ...
    def _tokenize_npz(self, idx):
        """
        Encode an npz file to no_pair_related token sequence.
        Replicates the logic from music_bert/mlm_dataset.py.
        """
