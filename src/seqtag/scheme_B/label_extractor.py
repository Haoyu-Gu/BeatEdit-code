"""
Label extraction for GECToR training data.
...
from config import (
    ...
    EMPTY_MARKER, END_MARKER, POSITION_OFFSET, POSITION_MIN, POSITION_MAX,
    MUSIC_TOKEN_MAX,
)
    ...
    for bar in source_info['bars']:
        ...
        for beat in bar['beats']:
            if s_beat_idx < len(target_info['beats']):
                ...
                beat_labels = extract_beat_labels(
                    beat['tokens'], t_beat['tokens']
                )
            else:
                beat_labels = [LABEL_KEEP] * len(beat['tokens'])
...
def extract_beat_labels(source_beat, target_beat):
    """
    Extract labels for a single beat by comparing source and target tokens.

    Three cases:
    1. Identical → all KEEP
    2. Equal length → token-by-token comparison
    3. Unequal length → note-level pitch matching
    """
    if source_beat == target_beat:
        return [LABEL_KEEP] * len(source_beat)

    # Both empty (shouldn't differ, but handle gracefully)
    if source_beat == [EMPTY_MARKER] and target_beat == [EMPTY_MARKER]:
        return [LABEL_KEEP]

    # Equal length: most common case (pitch shift, rhythm change)
    if len(source_beat) == len(target_beat):
        return _align_equal_length(source_beat, target_beat)

    # Unequal length: note-level matching (deletion or insertion)
    return _align_unequal_length(source_beat, target_beat)
...
def _align_equal_length(source_beat, target_beat):
    ...
    for s, t in zip(source_beat, target_beat):
        if s == t:
            ...
            labels.append(label_id_replace(t))
        elif is_control_token(s):
            labels.append(LABEL_KEEP)
        else:
            # Mixed types (shouldn't happen in well-formed data)
...
def _align_unequal_length(source_beat, target_beat):
    ...
    Strategy (from spec section 4.4):
    ...
    """
    # Handle empty beat edge cases
    if source_beat == [EMPTY_MARKER]:
        # Can't edit control token, just KEEP. Post-processing + iteration needed.
        return [LABEL_KEEP]

    if target_beat == [EMPTY_MARKER]:
        # Delete all note tokens, keep control tokens
        return [
            label_id_delete() if not is_control_token(t) else LABEL_KEEP
            for t in source_beat
        ]
    ...
    # Generate labels for note tokens
    labels = []
    exp_idx = 0
    ...
    # END_MARKER → KEEP
    if source_beat and source_beat[-1] == END_MARKER:
        labels.append(LABEL_KEEP)
    ...
    assert len(labels) == len(source_beat), \
        f"Labels length {len(labels)} != source length {len(source_beat)}"
...
def _apply_first_insert(labels, source_pairs, pitches_to_delete, common_pitches,
    ...
    # Find position token for this pitch in target encoding
    target_pairs = parse_note_pairs(target_beat)
    insert_pos_token = None
    for tpos, tval, tpitch in target_pairs:
        if tpitch == first_insert_pitch:
            insert_pos_token = tpos
            break

    if insert_pos_token is None or insert_pos_token > MUSIC_TOKEN_MAX:
        return
