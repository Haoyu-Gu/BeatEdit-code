...
Usage:
    ...
    accelerate launch train_gector.py --stage 3 --epochs 3 \
        --checkpoint checkpoints/gector/best_model \
...
from config import (
    NUM_LABELS, PAD_TOKEN, LABEL_PAD, DATA_DIR,
...
BERT_CHECKPOINT = os.path.join(
    ...
    'music_bert', 'checkpoints', 'music_bert_no_pair_related', 'best_model'
)


def parse_args():
    parser = argparse.ArgumentParser(description='Music GECToR Training')
    # Stage
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./checkpoints/gector')
...
def train():
    ...
    if is_main:
        print("=" * 60)
        print(f"Music GECToR Training - Stage {args.stage}")
        print("=" * 60)
