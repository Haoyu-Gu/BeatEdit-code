"""
Label extraction for GECToR training data (absolute_bundled encoding - Scheme D).
...
Key advantage of absolute_bundled:
...
- INSERT needs only 1 APPEND label per note
- DELETE needs only 1 DELETE label per note
- APPEND bundled token uses absolute position (no predecessor dependency)
"""
...
def _apply_first_insert(labels, source_notes, pitches_to_delete, common_pitches,
    ...
    In absolute_bundled, APPEND adds one complete bundled token = one complete note.
    The bundled token uses absolute position: bundled = abs_pitch × 81 + val.
    No predecessor dependency needed (unlike relative encoding in Scheme C).
    ...
    # Compute bundled token with absolute position (no predecessor dependency)
    insert_bundled = first_insert_pitch * PATTERN_NUM + first_insert_val
