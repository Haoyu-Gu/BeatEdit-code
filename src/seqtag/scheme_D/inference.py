"""
Music GECToR Inference Pipeline (absolute_bundled encoding - Scheme D)
...
Simplified vs no_pair_related:
- APPEND completes in 1 round (1 token = 1 note)
- No SHIFT handling needed
- Post-process simpler (each bundled token is self-contained)
- max_iterations reduced (2 rounds usually sufficient)

Usage:
    python inference.py --checkpoint checkpoints/gector_absolute_bundled/best_model \
...
from config import (
    ...
    EMPTY_MARKER, SPLIT_0, SPLIT_1, BAR_TOKEN, PATTERN_NUM,
    is_control_token, is_bundled_token, is_split_token,
...
def apply_labels(tokens, labels):
    ...
    APPEND inserts a bundled token after current token.
    DELETE removes the token.
    REPLACE replaces with a new bundled token.
    """
    result = []
    ...
    for token, label in zip(tokens, labels):
        ...
        if op == 'KEEP':
            ...
            result.append(value)    # add new bundled token after
...
def post_process(tokens):
    ...
    3. Re-encode with correct absolute positions
    """
    try:
...
def inference_single(model, input_tokens, device='cpu',
                     max_iterations=2, keep_confidence_bias=0.3,
                     error_threshold=0.5):
    """
    ...
    Args:
        ...
        device: torch device
        max_iterations: maximum correction rounds (default 2)
...
def main():
    parser = argparse.ArgumentParser(description='Music GECToR Inference (absolute_bundled)')
