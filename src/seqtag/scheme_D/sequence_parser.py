"""
Sequence Parser for absolute_bundled encoding (Scheme D).
...
def decode_beat(tokens):
    ...
    Args:
        tokens: beat token list (bundled tokens only, no SPLIT marker).
                e.g. [246, 810, 4050] or [] for empty beats.
    ...
    Examples:
        ...
        [(3, 3)]
        >>> decode_beat([810])  # 810 = 10*81 + 0 → pitch=10, val=0
        [(10, 0)]
        >>> decode_beat([])
        []
    """
...
def encode_beat(notes):
    ...
    Examples:
        ...
        [246, 850]  # 3*81+3=246, 10*81+40=850
        >>> encode_beat([])
        []
    """
    if len(notes) == 0:
