"""
GECToR Dataset and Collator (absolute_bundled encoding - Scheme D)

Loads npz piano roll files, tokenizes to absolute_bundled encoding,
...
# Import PianoRollTokenizer from the Scheme D encoding module
# (<repo>/src/encoding/scheme_D/my_tokenizer.py)
_SRC_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
MUSIC_BERT_DIR = os.path.join(_SRC_DIR, 'encoding', 'scheme_D')
...
class GECToRDataset(Dataset):
    """
    GECToR training dataset (absolute_bundled encoding).

    Each __getitem__ dynamically:
    1. Loads npz and encodes to absolute_bundled token sequence
    ...
    def __init__(
        ...
        # Create tokenizer (absolute_bundled config)
