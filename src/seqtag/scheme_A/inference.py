"""
Music GECToR Inference Pipeline (no_pair encoding)
...
Scheme A (no_pair): absolute positions, TRACK markers, SHIFT support.
Similar to Scheme B but with absolute position encoding.

Usage:
    python inference.py --checkpoint checkpoints/gector_no_pair/best_model \
...
from config import (
    ...
    TRACK0_START, TRACK1_START, BAR_TOKEN,
...
def inference_single(model, input_tokens, device='cpu',
    ...
    for iteration in range(max_iterations):
        ...
        for i in range(len(current_tokens)):
            # Control tokens always KEEP (including TRACK markers)
            if is_control_token(current_tokens[i]) or is_track_marker(current_tokens[i]):
...
def main():
    parser = argparse.ArgumentParser(description='Music GECToR Inference (no_pair)')
