"""
MusicGECToR Model (no_pair encoding)
...
- tag_predictor: 350-class tag prediction per token

Scheme A (no_pair): vocab=186 (185 + MASK), absolute position encoding
"""
...
def get_bert_config():
    """Create BERT config matching the pretrained Music BERT (no_pair)."""
    return BertConfig(
...
class MusicGECToR(nn.Module):
    """
    GECToR-style model for music sequence correction (no_pair).

    Architecture:
        BERT encoder -> hidden states -> error_detector (2-class)
                                      -> tag_predictor (350-class)
    """
    ...
    def __init__(self, num_labels=NUM_LABELS, dropout=0.1, bert_config=None):
        ...
        # IMPORTANT: add_pooling_layer=False to avoid DDP unused parameter error
...
def load_pretrained_bert(checkpoint_path, model=None):
    ...
    # Load into model.bert (strict=False to handle missing pooler keys)
...
def compute_loss(detect_logits, tag_logits, detect_labels, tag_labels,
    ...
    Args:
        ...
        tag_labels: (batch, seq_len) values 0-349/-100
