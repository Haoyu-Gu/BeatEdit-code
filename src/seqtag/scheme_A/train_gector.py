"""
Music GECToR Training Script (no_pair encoding)
...
Usage:
    ...
    accelerate launch train_gector.py --stage 3 --epochs 3 \
        --checkpoint checkpoints/gector_no_pair/best_model \
...
BERT_CHECKPOINT = os.path.join(
    ...
    'music_bert_no_pair', 'checkpoints', 'music_bert_no_pair', 'best_model'
)
...
def parse_args():
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./checkpoints/gector_no_pair')
...
def train():
    ...
    if is_main:
        print("=" * 60)
        print(f"Music GECToR Training (no_pair) - Stage {args.stage}")
        print("=" * 60)
        print(f"Encoding: no_pair absolute position (vocab={VOCAB_SIZE}, labels={NUM_LABELS})")
