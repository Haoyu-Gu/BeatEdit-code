"""
Music GECToR Evaluation Script (no_pair encoding)
...
5. Pitch Accuracy (position token correction rate)
6. Pattern Accuracy (patch value correction rate)

Usage:
    python evaluate.py --checkpoint checkpoints/gector_no_pair/best_model \
...
from config import (
    ...
    is_position_token, is_patch_value, is_control_token,
...
def compute_metrics(source_tokens, target_tokens, predicted_tokens,
    ...
    # --- Pitch/Pattern Accuracy (from labels) ---
    pitch_correct = pitch_total = 0
    pattern_correct = pattern_total = 0
        ...
        if i < len(source_tokens):
            ...
            if is_position_token(tok):
                if tl != LABEL_KEEP:
                    ...
                    if pl == tl:
                        pitch_correct += 1
            elif is_patch_value(tok):
                if tl != LABEL_KEEP:
                    pattern_total += 1
                    if pl == tl:
                        pattern_correct += 1

    metrics['pitch_accuracy'] = pitch_correct / max(pitch_total, 1)
    metrics['pattern_accuracy'] = pattern_correct / max(pattern_total, 1)

    return metrics


def evaluate_model(model, data_dir, num_samples=1000, device='cpu',
                   max_iterations=3, keep_bias=0.3, error_threshold=0.5,
                   seed=42):
    """
    ...
    for idx in range(len(ds)):
        try:
            ...
            # (use first iteration's raw predictions)
...
def print_results(results):
    ...
    print("\n" + "=" * 60)
    print("Music GECToR Evaluation Results (no_pair)")
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description='Music GECToR Evaluation (no_pair)')
