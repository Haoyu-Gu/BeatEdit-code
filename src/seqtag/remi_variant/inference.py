"""
REMI Music GECToR Inference Pipeline

Multi-round iterative inference with:
- KEEP confidence bias
- Error detection thresholding
- Post-processing for REMI structure validity

Usage:
    python inference.py --checkpoint checkpoints/gector_remi_stage3/best_model \
...
from config import (
    ...
    BAR_TOKEN, BOS_TOKEN, EOS_TOKEN,
    is_special_token, is_position_token, is_pitch_token,
    is_velocity_token, is_duration_token, is_music_token,
    decode_label, TRAINING_DEFAULTS as TD,
    PITCH_MIN, PITCH_MAX, VELOCITY_MIN, VELOCITY_MAX,
    DURATION_MIN, DURATION_MAX, POSITION_MIN, POSITION_MAX,
)
    ...
    for token, label in zip(tokens, labels):
        ...
        if op == 'KEEP':
            ...
            if is_position_token(new_token):
                ...
                result.append(token)  # out of range -> keep
        elif op == 'APPEND':
...
def post_process(tokens):
    """
    Post-process edited REMI tokens to ensure structural validity.

    Validates:
    1. Notes follow Position -> [Pitch Velocity Duration]+ pattern
    2. Token values are in valid ranges
    3. No orphaned tokens (e.g., Velocity without Pitch)
    """
    try:
    ...
    for bar in parsed['bars']:
        ...
        for pos in bar['positions']:
            ...
            for note in pos['notes']:
                p = note['pitch_token']
                v = note['velocity_token']
                d = note['duration_token']
                # Validate ranges
                if is_pitch_token(p) and is_velocity_token(v) and is_duration_token(d):
                    valid_notes.append({
                        'pitch_token': p,
                        'velocity_token': v,
                        'duration_token': d,
            ...
            if valid_notes:
                ...
                for note in valid_notes:
                    if note['pitch_token'] not in seen:
                        seen.add(note['pitch_token'])
                        unique_notes.append(note)
                bar_data.append({
                    'position_token': pos['position_token'],
                    'notes': unique_notes,
                })
...
def inference_single(model, input_tokens, device='cpu',
                     ...
                     error_threshold=0.5):
    """
    GECToR-style iterative inference for a single REMI sequence.
    """
    model.eval()
...
def main():
    parser = argparse.ArgumentParser(description='REMI Music GECToR Inference')
