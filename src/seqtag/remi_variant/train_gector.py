"""
REMI Music GECToR Training Script
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=2,3 accelerate launch --num_processes=2 --main_process_port=29501 \
        ...
        --bert_checkpoint checkpoints/music_bert_remi/best_model \
        --output_dir checkpoints/gector_remi

    # Stage III fine-tuning
    CUDA_VISIBLE_DEVICES=2,3 accelerate launch --num_processes=2 --main_process_port=29501 \
        train_gector.py --stage 3 --epochs 3 \
        --checkpoint checkpoints/gector_remi/best_model \
        --output_dir checkpoints/gector_remi_stage3
"""
...
from config import (
    NUM_LABELS, PAD_TOKEN, LABEL_PAD, MIDI_DATA_DIR, VOCAB_SIZE,
    TRAINING_DEFAULTS as TD,
)
from model import MusicGECToR, load_pretrained_bert, compute_loss
from dataset import (
    REMIGECToRDataset, REMICollator, BucketBatchSampler, get_file_lists,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description='REMI Music GECToR Training')
    ...
    parser.add_argument('--output_dir', type=str, default='./checkpoints/gector_remi')
...
def train():
    ...
    if is_main:
        print("=" * 60)
        print(f"REMI Music GECToR Training - Stage {args.stage}")
        print("=" * 60)
        print(f"Encoding: REMI (vocab={VOCAB_SIZE}, labels={NUM_LABELS})")
    ...
    collator = REMICollator(max_length=args.max_len)
    ...
    # Gradient checkpointing to maximize batch size
...
def evaluate(model, val_loader, accelerator, args, max_batches=50):
    ...
    for i, batch in enumerate(val_loader):
        ...
        pred_labels = tag_logits.argmax(dim=-1)
        true_labels = batch['labels']
        mask = (true_labels != LABEL_PAD)
        pred_edit = (pred_labels != 0) & mask
        true_edit = (true_labels != 0) & mask
