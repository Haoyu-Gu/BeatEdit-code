"""
Music GECToR Training Script (with_pair encoding)
...
Usage:
    ...
    accelerate launch train_gector.py --stage 3 --epochs 3 \
        --checkpoint checkpoints/gector_with_pair/best_model \
...
BERT_CHECKPOINT = os.path.join(
    ...
    'music_bert_with_pair', 'checkpoints', 'music_bert_with_pair', 'best_model'
)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Music GECToR Training (with_pair)')
    # Stage
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./checkpoints/gector_with_pair')
...
def train():
    ...
    if is_main:
        print("=" * 60)
        print(f"Music GECToR Training (with_pair) - Stage {args.stage}")
        print("=" * 60)
        print(f"Encoding: with_pair bundled (vocab={VOCAB_SIZE}, labels={NUM_LABELS})")
