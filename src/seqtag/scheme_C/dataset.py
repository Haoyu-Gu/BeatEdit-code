"""
GECToR Dataset and Collator (with_pair encoding)

Loads npz piano roll files, tokenizes to with_pair bundled encoding,
...
# Import PianoRollTokenizer from the Scheme C encoding module
# (<repo>/src/encoding/scheme_C/my_tokenizer.py)
_SRC_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
MUSIC_BERT_DIR = os.path.join(_SRC_DIR, 'encoding', 'scheme_C')
...
class GECToRDataset(Dataset):
    """
    GECToR training dataset (with_pair encoding).

    Each __getitem__ dynamically:
    1. Loads npz and encodes to with_pair bundled token sequence
    ...
    def __init__(
        ...
        # Create tokenizer (with_pair config)
    ...
    def _tokenize_npz(self, idx):
        ...
        for i in range(num_measures):
            ...
            for beat_idx in range(num_beats):
                ...
                # Track 0 (high voice) - with_pair: SPLIT_0
                p0 = beat_measure[:2]
                ...
                # Track 1 (low voice) - with_pair: SPLIT_1
                p1 = beat_measure[2:]
