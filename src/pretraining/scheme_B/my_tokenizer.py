...
- Relative-position compression encoding
...
class PianoRollTokenizer:
    ...
    2. Compress the token sequence using relative-position encoding.
    ...
    Args:
        ...
        pattern_num: offset for relative-position tokens (default 81).
    ...
    def encode(
        ...
        """Full encoding pipeline: piano roll -> compressed tokens.

        Args:
            image: dual-channel piano roll of shape (2, 88, t);
                ch0 = sustain, ch1 = onset.
            empty_marker_id: empty-beat marker ID.
            end_marker_id: end-of-non-empty-beat marker ID.
    ...
    def decode(
        ...
        """Full decoding pipeline: compressed tokens -> piano roll.
    ...
    def compress_tokens(
        ...
        """Compress a token sequence using relative-position encoding.
        ...
        Encoding format:
            non-empty: [81+rel_pos0, val0, 81+rel_pos1, val1, ..., end_marker]
                       - the first position is relative to 0 (i.e. absolute).
                       - subsequent positions are relative to the previous
                         nonzero position.
            empty:     [empty_marker]
        """
    ...
    def compress_tokens_velocity(
        ...
        """Compress a velocity-aware token sequence using relative-position encoding.
        ...
        Encoding format:
            non-empty: [81+rel_pos0, token0, velocity0+offset, 81+rel_pos1, token1, velocity1+offset, ..., end_marker]
            empty:     [empty_marker]
        """
        ...
        for beat_idx in range(pattern_matrix.shape[0]):
            ...
            if len(non_zero_indices) == 0:
                ...
                # Append [relative position, token value, velocity value] triples
                for idx in non_zero_indices:
                    relative_pos = idx - prev_pos
                    position_marker = self.pattern_num + relative_pos
    ...
    def decompress_tokens(
        ...
        """Decompress a token sequence (relative-position encoding -> token matrix).
        ...
        while i < len(compressed_sequence):
            ...
            if current_token == empty_marker_id:
                ...
                # Non-empty beat - the current token is the first position marker
                ...
                while i < len(compressed_sequence):
                    ...
                    if token == empty_marker_id:
                        # Start of the next beat (not consumed)
                        break
    ...
    def decompress_tokens_velocity(
        ...
        Args:
            compressed_sequence: compressed token sequence, formatted as
                [81+rel_pos0, token0, velocity0+offset, ..., end_marker] or [empty_marker].
            empty_marker_id: empty-beat marker ID.
            end_marker_id: end-of-non-empty-beat marker ID.
