...
复用 no_pair_related 编码的 tokenization 逻辑，在 note token 上施加 MLM masking。
"""
import os
import numpy as np
import torch
...
class MLMDataset(Dataset):
    ...
    从 npz 文件加载 piano roll，编码为 no_pair_related token 序列，
    ...
    def _tokenize_npz(self, idx):
        """将 npz 文件编码为 no_pair_related token 序列"""
        ...
        for i in range(num_measures):
            ...
            for beat_idx in range(num_beats):
                ...
                # Track 0 (高声部) - no_pair_related 无需 track marker
                p0 = beat_measure[:2]
    ...
    def _apply_mlm_mask(self, tokens):
        ...
        只 mask note tokens (0-168)，不 mask 控制 token (169+)。
