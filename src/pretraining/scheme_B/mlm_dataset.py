"""
Music BERT MLM Dataset

Reuses the no_pair_related tokenization logic and applies MLM masking on note tokens.
"""
import os
import numpy as np
import torch
...
class MLMDataset(Dataset):
    """MLM pretraining dataset.

    Loads a piano roll from an npz file, encodes it into a no_pair_related
    token sequence, then applies MLM masking on note tokens (0-168).
    """
    ...
    def _tokenize_npz(self, idx):
        """Encode an npz file into a no_pair_related token sequence."""
        ...
        for i in range(num_measures):
            ...
            for beat_idx in range(num_beats):
                ...
                # Track 0 (upper voice) - no_pair_related needs no track marker
                p0 = beat_measure[:2]
                ...
                comp0 = self.tokenizer.compress_tokens(
                    ...
                    end_marker_id=self.tc.end_marker_id,
                )
    ...
    def _apply_mlm_mask(self, tokens):
        ...
        Only note tokens (0-168) are masked; control tokens (169+) are not.
        Note tokens cover patch values (0-80) and relative positions (81-168).
