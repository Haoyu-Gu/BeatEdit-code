...
- Absolute-position compression encoding
...
class PianoRollTokenizer:
    ...
    2. Compress the token sequence using absolute-position encoding.
    ...
    Args:
        ...
        pattern_num: offset for position tokens (default 81).
    ...
    def encode(
        ...
        compressed = self.compress_tokens(tokens, track_marker_id=start_marker_id)
        return compressed
    ...
    def decode(
        ...
        Args:
            ...
            start_marker_id: track start marker ID.

        Returns:
            image: dual-channel piano roll of shape (2, 88, t).
        """
        tokens = self.decompress_tokens(compressed_sequence, track_marker_id=start_marker_id)
    ...
    def compress_tokens(
        ...
        """Compress a token sequence using absolute-position encoding.

        Args:
            token_matrix: token matrix of shape (num_beats, beats_length).
            track_marker_id: track start marker.

        Returns:
            compressed_sequence: compressed 1-D sequence.

        Encoding format:
            non-empty: [track_marker] [abs_pos0] [token0] [abs_pos1] [token1] ...
        ...
        for beat_tokens in token_matrix:
            ...
            if len(non_zero_indices) == 0:
                ...
                # Append [absolute position, token value] pairs
    ...
    def decompress_tokens(
        ...
        """Decompress a token sequence (absolute-position encoding -> token matrix).
        ...
        while i < len(compressed_sequence):
            ...
            if current_token == track_marker_id:
                ...
                # Read position-token pairs until the next beat starts
                while i < len(compressed_sequence):
                    ...
                    # Check whether this is the start of the next beat (start_marker or empty_marker)
