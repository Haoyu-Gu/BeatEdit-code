"""
Piano Roll Tokenizer - piano-roll encoder/decoder (absolute_bundled scheme - Scheme D).

Centralizes the encode/decode logic between a piano roll and tokens, including:
- Patch-based tokenization (ternary rhythm pattern encoding)
- Absolute-position bundled encoding (bundled = abs_pitch x 81 + val)
- Dual-channel piano-roll reconstruction

Difference from with_pair (Scheme C):
- compress_tokens: uses absolute position instead of relative position.
- decompress_tokens: decodes the absolute position directly, no state
  accumulation needed.
"""
...
class PianoRollTokenizer:
    """Token encoder/decoder for a piano roll (absolute_bundled).
    ...
    2. Compress the token sequence using absolute-position bundled encoding.
    3. Support a full encode-decode round-trip.

    Args:
        patch_h: patch height (default 1).
        patch_w: patch width (default 4, i.e. 4 time steps).
        pattern_num: number of ternary patterns (default 81).
        beats_length: image height, i.e. the number of piano keys (default 88).
    ...
    def encode(
        ...
        """Full encoding pipeline: piano roll -> compressed tokens (absolute-position bundled encoding).
    ...
    def decode(
        ...
        """Full decoding pipeline: compressed tokens -> piano roll (absolute-position bundled encoding).
    ...
    def compress_tokens(
        ...
        """Compress a token sequence using absolute-position bundled encoding (absolute position x pattern_num + patch value).

        Difference from with_pair (Scheme C): uses absolute position instead
        of relative position; each bundled token is encoded independently,
        with no need to track the previous position.
        ...
        Encoding format:
            non-empty: [split_marker] [bundled_0] [bundled_1] ...
                       bundled = absolute_pitch * pattern_num + token_value
            empty:     [empty_marker]
        """
        ...
        for beat_tokens in token_matrix:
            ...
            if len(non_zero_indices) == 0:
                ...
                # Use absolute-position bundled encoding (no need to track the previous position)
                for idx in non_zero_indices:
                    token_value = int(beat_tokens[idx])
                    bundled = int(idx * self.pattern_num + token_value)
    ...
    def decompress_tokens(
        ...
        """Decompress a token sequence (absolute-position bundled encoding -> token matrix).

        Difference from with_pair (Scheme C): decodes the absolute position
        directly from the bundled token, with no need to accumulate a
        relative position.
        ...
        while i < len(compressed_sequence):
            ...
            if current_token == empty_marker_id:
                ...
                # Read bundled tokens until a special marker (>= empty_marker_id) is hit
                while i < len(compressed_sequence):
                    ...
                    # Decode the absolute-position bundled token (direct decode, no accumulation)
                    abs_pos = int(next_token) // self.pattern_num
