"""
Music BERT MLM dataset (absolute_bundled encoding scheme - Scheme D).

Reuses the absolute_bundled tokenization logic and applies MLM masking on
bundled tokens.
"""
import os
import numpy as np
...
class MLMDataset(Dataset):
    """MLM pretraining dataset (absolute_bundled).

    Loads a piano roll from an npz file, encodes it into an absolute_bundled
    token sequence, then applies MLM masking on bundled tokens (0-7127).
    """
    ...
    def __init__(
        ...
        # Load the length cache (shared with with_pair's cache, since the sequence lengths match)
    ...
    def _tokenize_npz(self, idx):
        """Encode an npz file into an absolute_bundled token sequence."""
        ...
        for i in range(num_measures):
            ...
            for beat_idx in range(num_beats):
                ...
                # Track 0 (upper voice) - absolute_bundled uses SPLIT_0
                p0 = beat_measure[:2]
                ...
                # Track 1 (lower voice) - absolute_bundled uses SPLIT_1
                p1 = beat_measure[2:]
