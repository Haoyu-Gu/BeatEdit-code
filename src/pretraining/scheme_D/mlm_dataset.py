"""
Music BERT MLM Dataset (absolute_bundled 编码方案 - Scheme D)

复用 absolute_bundled 编码的 tokenization 逻辑，在 bundled token 上施加 MLM masking。
"""
import os
import numpy as np
import torch
...
class MLMDataset(Dataset):
    """
    MLM 预训练数据集 (absolute_bundled)

    从 npz 文件加载 piano roll，编码为 absolute_bundled token 序列，
    ...
    def __init__(
        ...
        # 加载长度缓存 (共用 with_pair 的缓存，因为序列长度相同)
    ...
    def _tokenize_npz(self, idx):
        """将 npz 文件编码为 absolute_bundled token 序列"""
        ...
        for i in range(num_measures):
            ...
            for beat_idx in range(num_beats):
                ...
                # Track 0 (高声部) - absolute_bundled 用 SPLIT_0
                p0 = beat_measure[:2]
                ...
                # Track 1 (低声部) - absolute_bundled 用 SPLIT_1
                p1 = beat_measure[2:]
