"""
CPWord Music BERT MLM Pretraining

Uses CompoundEmbedding + BertEncoder + 5 MLM heads with Accelerate for distributed training.
Masks music content compound tokens (Bar, Position, Note), NOT special tokens (BOS/EOS/PAD).

Usage:
    CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --num_processes=4 --main_process_port=29502 \
        train_mlm.py --epochs 30 --batch_size 64 --output_dir checkpoints/music_bert_cp
"""
...
from config import MIDI_DATA_DIR, SUB_VOCAB_SIZES, TRAINING_DEFAULTS as TD
from dataset import CPMLMDataset, CPMLMCollator, get_file_lists
from model import CPBertForMaskedLM
...
def train():
    parser = argparse.ArgumentParser(description='CPWord Music BERT MLM Pre-training')
    parser.add_argument('--epochs', type=int, default=30)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--output_dir', type=str, default='./checkpoints/music_bert_cp')
    ...
    if is_main:
        print("=" * 60)
        print("CPWord Music BERT MLM Pre-training")
        print("=" * 60)
        print(f"Encoding: CPWord (sub-vocabs={SUB_VOCAB_SIZES})")
    ...
    train_dataset = CPMLMDataset(
        file_list=train_files,
        data_dir=args.data_dir,
        max_len=args.max_len,
    )
    eval_dataset = CPMLMDataset(
        file_list=test_files,
        data_dir=args.data_dir,
        max_len=args.max_len,
    )

    collator = CPMLMCollator(max_length=args.max_len)
    ...
    # Model
    model = CPBertForMaskedLM(sub_vocab_sizes=SUB_VOCAB_SIZES, hidden_size=512)
    ...
    optimizer = torch.optim.AdamW(
        optimizer_grouped_parameters, lr=args.lr, betas=(0.9, 0.999), eps=1e-8
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            batch_masked = (batch['mlm_labels'][:, :, 0] != -100).sum().item()
                ...
                if global_step % args.eval_every == 0:
                    ...
                    if is_main:
                        ...
                        if eval_loss < best_eval_loss:
                            ...
                            save_model(model, accelerator, args.output_dir, 'best_model')
                            print(f"  [Best] Saved to {args.output_dir}/best_model")
                    model.train()

                if global_step % args.save_every == 0 and is_main:
                    save_model(model, accelerator, args.output_dir, f'step_{global_step}')
                    print(f"  [Save] step_{global_step}")
    ...
    # Final save
    if is_main:
        save_model(model, accelerator, args.output_dir, 'final_model')
...
def evaluate(model, eval_loader, accelerator, max_batches=50):
    ...
    for i, batch in enumerate(eval_loader):
        ...
        masked = (batch['mlm_labels'][:, :, 0] != -100).sum().item()
...
def save_model(model, accelerator, output_dir, name):
    """Save model weights (unwrapped from DDP/Accelerate)."""
