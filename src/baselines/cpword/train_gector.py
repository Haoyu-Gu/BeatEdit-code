"""
CPWord Music GECToR Training Script

Accelerate-based multi-GPU training with factored heads:
- Stage I: Synthetic error data (freeze encoder -> unfreeze -> fine-tune)
- Stage III: Mixed clean sample fine-tuning (prevent over-correction)

Usage:
    # Stage I training
    CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --num_processes=4 --main_process_port=29502 \
        train_gector.py --stage 1 --epochs 20 \
        --bert_checkpoint checkpoints/music_bert_cp/best_model \
        --output_dir checkpoints/gector_cp

    # Stage III fine-tuning
    CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --num_processes=4 --main_process_port=29502 \
        train_gector.py --stage 3 --epochs 3 --eval_every 200 \
        --checkpoint checkpoints/gector_cp/best_model \
        --output_dir checkpoints/gector_cp_stage3
"""
...
from config import (
    NUM_ACTIONS, ACTION_KEEP, LABEL_PAD,
    MIDI_DATA_DIR, SUB_VOCAB_SIZES,
    TRAINING_DEFAULTS as TD,
)
from model import CPMusicGECToR, load_pretrained_bert, compute_loss
from dataset import (
    CPGECToRDataset, CPGECToRCollator, get_file_lists,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description='CPWord Music GECToR Training')
    ...
    parser.add_argument('--lambda_sub', type=float, default=TD['lambda_sub'])
    ...
    parser.add_argument('--output_dir', type=str, default='./checkpoints/gector_cp')
...
def build_model(args):
    ...
    elif args.bert_checkpoint and os.path.exists(args.bert_checkpoint):
...
def train():
    ...
    if is_main:
        print("=" * 60)
        print(f"CPWord Music GECToR Training - Stage {args.stage}")
        print("=" * 60)
        print(f"Encoding: CPWord (sub-vocabs={SUB_VOCAB_SIZES}, actions={NUM_ACTIONS})")
    ...
    collator = CPGECToRCollator(max_length=args.max_len)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            with accelerator.accumulate(model):
                detect_logits, action_logits, sub_logits = model(
                    compound_ids=batch['compound_ids'],
                    attention_mask=batch['attention_mask'],
                )

                total_loss, action_loss, sub_loss, detect_loss = compute_loss(
                    detect_logits, action_logits, sub_logits,
                    batch['detect_labels'], batch['action_labels'],
                    batch['sub_token_labels'],
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.log_every == 0 and is_main:
                    ...
                    avg_action = epoch_action_loss / epoch_steps
                    avg_sub = epoch_sub_loss / epoch_steps
                    avg_det = epoch_detect_loss / epoch_steps
                    lr = scheduler.get_last_lr()[0]
                    elapsed = time.time() - t0
                    print(
                        f"E{epoch+1} S{global_step} | "
                        f"Loss {avg_total:.4f} (act={avg_action:.4f} sub={avg_sub:.4f} det={avg_det:.4f}) | "
                        f"LR {lr:.2e} | {elapsed:.0f}s"
                    )
                    if tb_writer:
                        tb_writer.add_scalar('train/total_loss', avg_total, global_step)
                        tb_writer.add_scalar('train/action_loss', avg_action, global_step)
                        tb_writer.add_scalar('train/sub_loss', avg_sub, global_step)
...
def evaluate(model, val_loader, accelerator, args, max_batches=50):
    ...
    for i, batch in enumerate(val_loader):
        ...
        loss, action_loss, sub_loss, detect_loss = compute_loss(
            detect_logits, action_logits, sub_logits,
            batch['detect_labels'], batch['action_labels'],
            batch['sub_token_labels'],
            keep_weight=args.keep_weight,
        ...
        # Edit F1 based on action predictions
        pred_actions = action_logits.argmax(dim=-1)
        true_actions = batch['action_labels']
        mask = (true_actions != LABEL_PAD)
        pred_edit = (pred_actions != ACTION_KEEP) & mask
        true_edit = (true_actions != ACTION_KEEP) & mask
