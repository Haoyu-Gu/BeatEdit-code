"""
FELIX-Music Perturbation System.

4-level perturbation that operates ONLY on accompaniment (Track 1) beats.
Melody (Track 0) is never modified.

Reuses single-note perturbation primitives from GECToR, but completely
rewrites the sequence-level logic with beat-level and passage-level operations.
...
from configs.config import (
    MAX_PITCH, PATTERN_NUM, EMPTY_MARKER, SPLIT_1,
)
    ...
    for i, beat in enumerate(accomp_beats):
        if i in edit_indices:
            ...
            if changed:
                ...
                if len(new_tokens) == 0:
                    new_beat['split_id'] = None
                elif new_beat.get('split_id') is None:
                    new_beat['split_id'] = SPLIT_1
...
def _perturb_level_2(accomp_beats):
    ...
    if n >= 3:
        ...
        for i in range(start, start + passage_len):
            ...
            if len(new_notes) > 0:
                ...
                if result[i].get('split_id') is None and len(new_tokens) > 0:
                    result[i]['split_id'] = SPLIT_1
                changed_mask[i] = True
    ...
    empty_indices = [i for i in range(n) if len(result[i]['tokens']) == 0]
    if empty_indices:
...
def _perturb_level_3(accomp_beats):
    ...
    non_empty_indices = [i for i in range(n) if len(result[i]['tokens']) > 0]
    if non_empty_indices:
        ...
        for i in delete_indices:
            result[i] = {
                'tokens': [],
                'split_id': None,
                'start_idx': result[i].get('start_idx', 0),
                'end_idx': result[i].get('end_idx', 0),
            }
            changed_mask[i] = True
    ...
    for i in simplify_indices:
        ...
        if len(notes) > 1:
            ...
            result[i]['tokens'] = new_tokens
            if len(new_tokens) > 0 and result[i].get('split_id') is None:
                result[i]['split_id'] = SPLIT_1
            changed_mask[i] = True
