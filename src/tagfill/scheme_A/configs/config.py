"""
FELIX-Music Configuration (Scheme A: no_pair)

Token constants (from no_pair encoding), FELIX token-level label system,
and model/training hyperparameters.

Encoding scheme: no_pair (absolute position, 2 tokens per note, vocab=186)
  Each note = [abs_position_token][value_token]
  Position tokens: 81-168 (81 + pitch_index, absolute)
  Value tokens: 0-80 (81 ternary patterns)
  Beat structure: [TRACK_MARKER][pos][val]..., empty = [TRACK_MARKER][0]
...
# Track markers (no_pair uses explicit track markers)
