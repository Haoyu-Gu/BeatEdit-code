...
from configs.config import (
    LABEL_KEEP, LABEL_DELETE, LABEL_REPLACE, MASK_TOKEN, PAD_TOKEN,
...
class FELIXPipeline:
    ...
    def generate(
        ...
        for iteration in range(num_iterations):
            ...
            # Fix 1: Enforce (pos, val) pair label consistency
            labels = self._enforce_pair_consistency(current_tokens, labels)
            ...
            # Step 4: Fix 2 — normalize relative positions after Inserter
            current_tokens = self._normalize_relative_positions(current_tokens)

            # Step 5: Harmony-aware post-processing (optional)
    ...
    def _enforce_pair_consistency(self, tokens, labels):
        """
        Fix 1: For 2-token-per-note encoding, force consistent labels on (pos, val) pairs.

        When Tagger predicts mismatched labels for a note's pos and val tokens
        (e.g., DELETE pos but KEEP val), the skeleton becomes structurally invalid.
        This forces both tokens in a pair to share the same label.

        Priority: KEEP > REPLACE > DELETE (conservative: preserve information).
        """
        i = 0
        while i < len(tokens) - 1:
            if is_position_token(tokens[i]) and is_patch_value(tokens[i + 1]):
                l1, l2 = labels[i], labels[i + 1]
                if l1 != l2:
                    # Resolve conflict conservatively
                    if l1 == LABEL_KEEP or l2 == LABEL_KEEP:
                        labels[i] = labels[i + 1] = LABEL_KEEP
                    elif l1 == LABEL_REPLACE or l2 == LABEL_REPLACE:
                        labels[i] = labels[i + 1] = LABEL_REPLACE
                    else:
                        labels[i] = labels[i + 1] = l1
                i += 2
            else:
                i += 1
        return labels

    def _normalize_relative_positions(self, token_sequence):
        """
        Fix 2: Re-encode each accompaniment beat to fix cascaded relative position errors.

        After Inserter fills MASKs, the relative positions may be inconsistent because
        each MASK was predicted independently. This decodes each beat's notes (using the
        cumulative relative positions) and re-encodes them with correct relative positions.
        """
        parsed = parse_sequence(token_sequence)
        melody_beats, accomp_beats = separate_tracks(parsed)
        ...
        for beat in accomp_beats:
            ...
            notes = decode_beat(toks)
            if len(notes) == 0:
                corrected_accomp.append(beat)
                continue

            # Filter out invalid pitches and re-encode
            valid_notes = [(p, v) for p, v in notes if 0 <= p <= 87 and 0 <= v <= 80]
            if len(valid_notes) == 0:
                corrected_accomp.append({'tokens': [EMPTY_MARKER],
                                         'start_idx': beat.get('start_idx', 0),
                                         'end_idx': beat.get('end_idx', 0)})
                any_changed = True
                continue
            ...
            try:
                new_tokens = encode_beat(valid_notes)
                if new_tokens != toks:
                    any_changed = True
                new_beat = dict(beat)
